...
/**
 * Response events that wallet-service publishes after processing commands.
 * Payment-service listens for these to continue the saga.
 */
public record WalletResponseEvent(
    ...
    public static WalletResponseEvent success(UUID correlationId, UUID walletId, UUID userId,
                                               ...
                                               String currency) {
        return new WalletResponseEvent(
                UUID.randomUUID(),
                correlationId,
                walletId,
                userId,
